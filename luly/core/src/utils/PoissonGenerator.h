...
namespace PoissonGenerator
{
    ...
    public:
        ...
        uint32_t randomInt(uint32_t maxInt)
        {
            return static_cast<uint32_t>(randomFloat() * maxInt);
        }
    ...
    {
        return GridPoint(static_cast<int>(P.x / cellSize), static_cast<int>(P.y / cellSize));
    }
    ...
    **/
    ...
    )
    {
        ...
        if (minDist < 0.0f)
        {
            minDist = sqrt(static_cast<float>(numPoints)) / static_cast<float>(numPoints);
        }
    ...
    Point sampleVogelDisk(uint32_t idx, uint32_t numPoints, float phi)
    {
        ...
        const float r = sqrtf(static_cast<float>(idx) + 0.5f) / sqrtf(static_cast<float>(numPoints));
    ...
    /**
        Return a vector of generated points
    **/
    std::vector<Point> generateVogelPoints(uint32_t numPoints, bool isCircle = true, float phi = 0.0f,
    ...
    **/
    ...
    )
    {
        ...
        const uint32_t gridSize = static_cast<uint32_t>(sqrt(numPoints));
        ...
        for (uint32_t x = 0; x != gridSize; x++)
        {
            for (uint32_t y = 0; y != gridSize; y++)
            {
                ...
                do
                {
                    const Point offs = generateRandomPointAround(Point(0, 0), jitterRadius, generator) - center + Point(
                        0.5f, 0.5f);
                    p = Point(static_cast<float>(x) / gridSize, static_cast<float>(y) / gridSize) + offs;
    ...
    namespace
    {
        ...
        float radicalInverse_VdC(uint32_t bits)
        {
            ...
            return static_cast<float>(static_cast<float>(bits) * 2.3283064365386963e-10); // / 0x100000000
        }

        Point hammersley2d(uint32_t i, uint32_t N)
        {
            return Point(static_cast<float>(i) / static_cast<float>(N), radicalInverse_VdC(i));
        }
    } // namespace
